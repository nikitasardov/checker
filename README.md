...
- Внутреннее состояние проверки привязано к `url`, поэтому изменение `name` не сбрасывает счетчик ошибок и корректно отображается в логах/уведомлениях после перезагрузки конфига.
